...
entity pitch_shift is
  port (
    ...
    output_phase : out ram_24bit;
    output_magnitude : out ram_24bit
       );
  ...
  begin
    if rising_edge(clk) then
      if rst = '1' then
        ...
        ram_index := to_integer(virtual_read_index(17 downto 8)); -- convert fractional frequency pointer to integer

        if ram_index < 1024 then -- boundary check
            ...
            -- Phase tracking math (Automatic Modulo 2*PI wrapping happens here)
            ...
            previous_phase(ram_index) <= input_phase(ram_index);
            output_phase(write_index) <= std_logic_vector(new_accumulated_phase(35 downto 12));

            -- move the magnitude to the new bin
            output_magnitude(write_index) <= input_magnitude(ram_index);
        else
            -- If the pitch shift pushes us past the available memory, output silence
            output_phase(write_index) <= (others => '0');
            output_magnitude(write_index) <= (others => '0');
        end if;
